...
USE master;
alter database college set multi_user with rollback immediate;
drop database college;
...
--in smss
/* IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'college')
BEGIN
    CREATE DATABASE college;
END;
GO
...
BEGIN
    ALTER DATABASE college SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE college;
END;
GO
*/
...
/* 
5. alter (to change the schema), # schema are design (columns, datatypes, constraints)
